#
# 采用LSTM+CTC实现不定长验证码识别：1-10位大写字母识别
# 参考来源：https://www.jianshu.com/p/7a2b227896a8
#
...
class LstmCtcOcr:
    ...
    def test(self, img_path=""):
        ...
        # 加载w,b参数
        saver.restore(self.sess, './model/LstmCtcOcr-25')

        if img_path == "":
            imgpath = os.path.join(os.getcwd(), './data/ctc_test')
            imgs = os.listdir(imgpath)
            wrong = 0
            for img in imgs:
                # lable = instance.pred_oneimage(os.path.join(imgpath, img), model=model)
                imag = os.path.join(imgpath, img)
                im = cv2.imread(imag)
                im = cv2.resize(im, (self.im_width, self.im_height))
                im = np.array([im[:, :, 0]], dtype=np.float32)
                im -= 147
                pred = self.sess.run(self.pred, feed_dict={self.x: im})
                sequence = self.sparseTensor2sequence(pred)
                lable = ''.join(sequence[0])
                # print(img)
                real = img.split("_")[1][:-4]
                if lable != real:
                    # print(real, lable)
                    wrong += 1
    ...
    def train(self):
        ...
        with tf.Session() as self.sess:
            ...
            # 加载w,b参数
            saver.restore(self.sess, './model/LstmCtcOcr-10')
            ...
            for epoch_num in range(self.epoch_max):
                ...
                if (epoch_num + 1) % self.save_epoch == 0:
                    saver.save(self.sess, './model/LstmCtcOcr', global_step=(epoch_num + 1))
    ...
    def dataset_opt(self, x_train_queue, y_train_queue):
        ...
        contents = tf.read_file('./data/ctc_train/' + queue)
    ...
    def getTrainDataset(self):
        train_data_list = os.listdir('./data/ctc_train/')
    ...
    def createCaptchaDataset(self):
        ...
        print("Generate train data: ", './data/ctc_train/')
        for i in range(self.im_total_num):
            ...
            im_path = './data/ctc_train/%d_%s.png' % (i, words_tmp)
            print(im_path)
            image.write(words_tmp, im_path)

        print("Generate test data: ", './data/ctc_test/')
        for i in range(int(self.im_total_num*0.1)):
            words_tmp = ''
            for j in range(random.randint(1, self.words_max_num)):
                words_tmp = words_tmp + random.choice(self.words)
            print(words_tmp, type(words_tmp))
            im_path = './data/ctc_test/%d_%s.png' % (i, words_tmp)
            print(im_path)
            image.write(words_tmp, im_path)
...
if __name__ == '__main__':
    ...
    opt_type = input("Please choose: 1.train  2.test  3.create_dataset ")
    instance = LstmCtcOcr()
    ...
    if opt_type == '1':
        ...
        # instance.test('./data/ctc_test/17_RJPA.png')
        # instance.test('./data/ctc_test/24_EAYVFWT.png')
        instance.test()
    ...
    # 将session持久化到内存中
        ...
        instance.test('./data/ctc_test/0_PIY.png')
